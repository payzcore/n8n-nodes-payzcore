        ...
        if (resource === 'payment') {
          ...
          if (operation === 'get') {
            ...
            if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(paymentId)) {
              throw new NodeOperationError(this.getNode(), 'Payment ID must be a valid UUID');
            }
          ...
          if (operation === 'cancel') {
            const paymentId = this.getNodeParameter('cancelPaymentId', i) as string;
            if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(paymentId)) {
              throw new NodeOperationError(this.getNode(), 'Payment ID must be a valid UUID');
            }
            const response = await payzCoreApiRequest.call(this, 'PATCH', `/payments/${paymentId}`, { action: 'cancel' });
            returnData.push({ json: response, pairedItem: { item: i } });
          }
